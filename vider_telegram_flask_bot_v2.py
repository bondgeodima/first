    ...
    if request.method == "POST":
        if 'update_id' in request.json:
            ...
            if isinstance(viber_request, ViberMessageRequest):
                ...
                if t == 'text':
                    d = {"A": "А", "B": "В", "C": "С", "E": "Е", "H": "Н", "I": "І", "K": "К", "M": "М", "O": "О",
                         "P": "Р",
                         "T": "Т", "X": "Х"}
                    print('send text')
                    connection = connection_db()
                    ...
                    for value in textArr:
                        ...
                        if caption is not None:
                            if caption[0:1] == '@':
                                ...
                                select_query = "SELECT number FROM avto.user WHERE chat_id = '" + str(
                                    chat_id) + "' LIMIT 1"
                                print(select_query)
                                cursor.execute(select_query)
                                mobile_records = cursor.fetchall()
                                ...
                                if messenger == 't':
                                    bot.send_photo(chat_id=chat_id_out, photo=open(src, "rb"))
                                    bot.send_message(chat_id=chat_id_out, text='#' + str(my_avto_number) + ' ' + caption)
                                if messenger == 'v':
                                    viber.send_messages(chat_id_out, [
                                        PictureMessage(text='#' + my_avto_number + ' ' + caption, media=message.media)
                                    ])
                                    viber.send_messages(chat_id_out, [
                                        TextMessage(text='#' + my_avto_number + ' ' + caption)
                                    ])
